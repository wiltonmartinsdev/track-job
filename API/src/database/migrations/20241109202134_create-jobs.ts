...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.createTable("jobs", (table) => {
		...
		table.text("payment_currency").notNullable();
		table.decimal("initial_salary", 10, 2).notNullable().defaultTo(0);
		table.decimal("current_salary", 10, 2).notNullable().defaultTo(0);
		...
		table
			...
			.notNullable()
			.defaultTo(knex.raw("(datetime('now', '-3 hours'))"));
		table
